...
type PropertyGraph<'v, 'e, 'vdata, 'edata when 'v : comparison and 'e : comparison> = {
    Graph: Graph<'v, 'e>
...
module PropertyGraph =
    ...
    let empty<'v, 'e, 'vdata, 'edata when 'v : comparison and 'e : comparison>: PropertyGraph<'v, 'e, 'vdata, 'edata> = {
        Graph = Graph.empty
        Vertices = Map.empty
        Edges = Map.empty
    }
    ...
    let isEmpty (pg: PropertyGraph<'v, 'e, 'vdata, 'edata>) =
        Graph.isEmpty pg.Graph

    /// Get the edges incident to a vertex, returning None if the vertex is not found.
    let getIncidentEdges vertex (pg: PropertyGraph<'v, 'e, 'vdata, 'edata>) =
        pg.Graph
    ...
    let neighbors vertex (pg: PropertyGraph<'v, 'e, 'vdata, 'edata>) =
        pg.Graph
    ...
    let degree vertex (pg: PropertyGraph<'v, 'e, 'vdata, 'edata>) =
        Graph.degree vertex pg.Graph
        
    /// Check if a vertex exists in the graph.
    let containsVertex vertex (pg: PropertyGraph<'v, 'e, 'vdata, 'edata>) =
        Graph.containsVertex vertex pg.Graph
        
    /// Check if an edge exists in the graph.
    let containsEdge edge (pg: PropertyGraph<'v, 'e, 'vdata, 'edata>) =
        Graph.containsEdge edge pg.Graph

    /// Get the endpoints of an edge, returning None if the edge is not found.
    let getEndpoints edge (pg: PropertyGraph<'v, 'e, 'vdata, 'edata>) =
        pg.Graph
    ...
    let isAdjacent vertex1 vertex2 (pg: PropertyGraph<'v, 'e, 'vdata, 'edata>) =
    ...
    let findEdges vertex1 vertex2 (pg: PropertyGraph<'v, 'e, 'vdata, 'edata>) =
        pg.Graph
    ...
    let addVertex vertex data (pg: PropertyGraph<'v, 'e, 'vdata, 'edata>) =
    ...
    let addEdge edge data vertex1 vertex2 (pg: PropertyGraph<'v, 'e, 'vdata, 'edata>) = result {
    ...
    let removeVertex vertex (pg: PropertyGraph<'v, 'e, 'vdata, 'edata>) =
    ...
    let removeEdge edge (pg: PropertyGraph<'v, 'e, 'vdata, 'edata>) =
    ...
    /// connected edges).
    let filterVertices predicate (pg: PropertyGraph<'v, 'e, 'vdata, 'edata>) =
        pg.Vertices
    ...
    let filterEdges predicate (pg: PropertyGraph<'v, 'e, 'vdata, 'edata>) =
        pg.Edges
    ...
    let inducedSubgraph vertexSet (pg: PropertyGraph<'v, 'e, 'vdata, 'edata>) =
    ...
    let bfs start predicate (pg: PropertyGraph<'v, 'e, 'vdata, 'edata>) =
    ...
    let setEdgeData edge data (pg: PropertyGraph<'v, 'e, 'vdata, 'edata>) =
    ...
    let mapVertices mapper (pg: PropertyGraph<'v, 'e, 'vdata, 'edata>) =
        let vertices = Map.map mapper pg.Vertices

        { pg with Vertices = vertices }

    /// Map a function over the edge data of the property graph.
    let mapEdges mapper (pg: PropertyGraph<'v, 'e, 'vdata, 'edata>) =
